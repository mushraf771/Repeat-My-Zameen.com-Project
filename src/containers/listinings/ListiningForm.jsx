...
const ListiningForm = (props) => {
    ...
    const { sale_type, home_type, city, state, price, area_size, area_type,
        ...
        bathrooms,
        keywords, } = formData;
    console.log(formData,'ye line 23 ki warning he');
    const [Loading, setLoading] = useState(false);
    const onChange = (e) =>
        setFormData({ ...formData, [e.target.name]: e.target.value });
    // console.log(formData, 'check data line 27 on change ListingsForm');
const onSubmit = e => {
        e.preventDefault()
        const config = {
            headers: {
    ...
    axios.post(`/properties/search/`, {
        ...
        bedrooms,
        bathrooms,
        keywords }, config).then(res => {
            setLoading(false);
            console.log(res.data, 'line 43 res.data checking  To Set ',);
            props.setListings(res.data);
            window.scrollTo(0, 0);
        }).catch(err => {
            setLoading(false);
            console.error(err,'ERRORS  Line 47');
            window.scrollTo(0, 0);
        })
    }
    return (
...
ListiningForm.propTypes = {
    setListings: PropTypes.func.isRequired
};

export default ListiningForm